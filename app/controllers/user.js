...
var User = {
	create: function(req, res, next){
		...
		UserModel.create({nickname: req.body.username, password: req.body.password, email: req.body.email, phonenumber: req.body.phonenumber}).then(function(user){
	...
	login: function(req, res, next){
		...
		UserModel.findOne({nickname: req.body.username}).then(function(user){
			...
			if(user.password != req.body.password) return res.json({message : 'Login failed', error: 'Error'});
